...
__global__ void init_fault_cu(Fault F){
  int j = blockIdx.x * blockDim.x + threadIdx.x;
  int k = blockIdx.y * blockDim.y + threadIdx.y;
  int j1 = j + 3;
  int k1 = k + 3;
...
void nc_read_init_stress(Fault F){
  ...
  real_t *var = (real_t*) malloc(sizeof(real_t)*nj*nk);
  ...
  size_t size = nj*nk*sizeof(real_t);
