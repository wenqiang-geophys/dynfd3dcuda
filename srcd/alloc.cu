...
void alloc_fault(Fault *F){
  ...
  //if (hostParams.Friction_type == 3){
    ...
    cudaMemset(F->TP_T, 0, sizeof(real_t)*TP_n*nj*nk);
    cudaMemset(F->TP_P, 0, sizeof(real_t)*TP_n*nj*nk);
  //}
  ...
  cudaMemset(F->init_t0_flag, 0, sizeof(int)  *nj*nk);
  cudaMemset(F->init_t0,      0, sizeof(real_t)*nj*nk);
  cudaMemset(F->slip,         0, sizeof(real_t)*nj*nk);
  cudaMemset(F->hslip,        0, sizeof(real_t)*nj*nk);
