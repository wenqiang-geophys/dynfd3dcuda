...
__global__ void wave2fault_cu(Wave w, Fault f, realptr_t M)
{
  return;
  // transform
...
__global__ void fault2wave_cu(Wave w, Fault f, realptr_t M)
{
  // transform
  //          fault (T11 (at i0), ...
  //                 T21, ..., F->T31, ...)
...
void wave2fault(Wave w, Fault f, realptr_t M)
{
  dim3 block(256, 1, 1);
  dim3 grid(
      (hostParams.nj + block.x-1)/block.x,
      (hostParams.nk + block.y-1)/block.y, 1);
  wave2fault_cu <<<grid, block>>> (w, f, M);
}
void fault2wave(Wave w, Fault f, realptr_t M)
{
  dim3 block(256, 1, 1);
  dim3 grid(
      (hostParams.nj + block.x-1)/block.x,
      (hostParams.nk + block.y-1)/block.y, 1);
  fault2wave_cu <<<grid, block>>> (w, f, M);
}
