...
__global__
void cal_rup_sensor_D_cu(Fault F)
{
...
__global__
void cal_rup_sensor_magn_cu(Fault F)
{
  ...
  real_t *p = F.Ts1;
  real_t *Dy = F.rup_sensor_Dy;
  real_t *Dz = F.rup_sensor_Dz;
      ...
      gk1 >= par.Fault_grid[2]+3 && gk1 <= par.Fault_grid[3]-3 )
  {
    ...
    F.rup_sensor[pos] = Dp_magn/(p0+1e-16) + 1e-16;

    if (F.united[pos]) F.rup_sensor[pos] = 0; 
  }
}

void cal_rup_sensor(Fault F)
{
  dim3 block(8, 8, 1);
  dim3 grid(
  ...
  cal_rup_sensor_D_cu <<<grid, block>>> (F);
  cal_rup_sensor_magn_cu <<<grid, block>>> (F);
}
