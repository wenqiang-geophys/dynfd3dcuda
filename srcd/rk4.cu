...
__global__ void fault_rk_stage2_cu(Fault f, int irk){
...
__global__ void fault_rk_cu(Fault F, int irk){
  ...
  real_t *f_mT21 = F.mW + 3 * nyz2;
  real_t *f_mT22 = F.mW + 4 * nyz2;
  real_t *f_mT23 = F.mW + 5 * nyz2;
  real_t *f_mT31 = F.mW + 6 * nyz2;
  real_t *f_mT32 = F.mW + 7 * nyz2;
  real_t *f_mT33 = F.mW + 8 * nyz2;

  real_t *f_tVx  = F.tW + 0 * nyz2;
  real_t *f_tVy  = F.tW + 1 * nyz2;
  real_t *f_tVz  = F.tW + 2 * nyz2;
  real_t *f_tT21 = F.tW + 3 * nyz2;
  real_t *f_tT22 = F.tW + 4 * nyz2;
  real_t *f_tT23 = F.tW + 5 * nyz2;
  real_t *f_tT31 = F.tW + 6 * nyz2;
  real_t *f_tT32 = F.tW + 7 * nyz2;
  real_t *f_tT33 = F.tW + 8 * nyz2;
...
__global__ void state_rk_cu(Fault F, int irk){
...
void fault_rk(Fault F, int irk)
{
  ...
  fault_rk_cu        <<<grid2, block2>>> (F, irk);
  fault_rk_stage2_cu <<<grid2, block2>>> (F, irk);
  return;
}

void state_rk(Fault F, int irk)
{
  // launch as two dimension
  dim3 block2(16, 8, 1);
  dim3 grid2(
  ...
  state_rk_cu <<<grid2, block2>>> (F, irk);
  return;
}
