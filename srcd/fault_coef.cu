...
__global__ void init_fault_coef_cu(real_t *M, Fault FC)
{
...
#pragma unroll 3
    ...
    FC.rho_f[j + k * ny + 0 * ny * nz] = RHO[pos-ny*nz];
    FC.rho_f[j + k * ny + 1 * ny * nz] = RHO[pos+ny*nz];

    FC.lam_f[j + k * ny + 0 * ny * nz] = LAM[pos-ny*nz];
    FC.lam_f[j + k * ny + 1 * ny * nz] = LAM[pos+ny*nz];

    FC.mu_f [j + k * ny + 0 * ny * nz] = MIU[pos-ny*nz];
    FC.mu_f [j + k * ny + 1 * ny * nz] = MIU[pos+ny*nz];
    ...
    //lam = LAM[pos];
    //miu = MIU[pos];
    lam = FC.lam_f[j + k * ny + 0 * ny * nz];
    miu = FC.mu_f [j + k * ny + 0 * ny * nz];
    ...
    //lam = LAM[pos];
    //miu = MIU[pos];
    lam = FC.lam_f[j + k * ny + 1 * ny * nz];
    miu = FC.mu_f [j + k * ny + 1 * ny * nz];
...
#pragma unroll 3
    ...
    if(k == nz-4){
      for (int ii = 0; ii < 3; ++ii){
        for (int jj = 0; jj < 3; ++jj){
          FC.matVytoVxfm[j*3*3+3*ii+jj] = mat[ii][jj];
        }
      }
    }
    ...
    if(k == nz-4){
      for (int ii = 0; ii < 3; ++ii){
        for (int jj = 0; jj < 3; ++jj){
          FC.matT1toVxfm[j*3*3+3*ii+jj] = mat[ii][jj];
        }
      }
    }
    // Plus side
    ...
    if(k == nz-4){
      for (int ii = 0; ii < 3; ++ii){
        for (int jj = 0; jj < 3; ++jj){
          FC.matVytoVxfp[j*3*3+3*ii+jj] = mat[ii][jj];
        }
      }
    }
    matmul3x3(mat3, B0, mat);
    if(k == nz-4){
      for (int ii = 0; ii < 3; ++ii){
        for (int jj = 0; jj < 3; ++jj){
          FC.matT1toVxfp[j*3*3+3*ii+jj] = mat[ii][jj];
        }
      }
    }
...
//    }
    ...
    if ((nz - 4) == k)
    {
      ...
      vec2[0] = FC.x_et[j + k * ny];
      vec2[1] = FC.y_et[j + k * ny];
      vec2[2] = FC.z_et[j + k * ny];
      ...
      for (int ii = 0; ii < 3; ++ii){
        for (int jj = 0; jj < 3; ++jj){
          FC.matT1toVxfm[j*9+3*ii+jj] = Tovert[ii][jj];
          FC.matVytoVxfm[j*9+3*ii+jj] = matPlus2Min3f[ii][jj];
        }
      }
      ...
      for (int ii = 0; ii < 3; ++ii){
        for (int jj = 0; jj < 3; ++jj){
          FC.matT1toVxfp[j*9+3*ii+jj] = Tovert[ii][jj];
          FC.matVytoVxfp[j*9+3*ii+jj] = matMin2Plus3f[ii][jj];
        }
      }
...
//#define DEBUG
      ...
      if (par.nj/2 == j){
          ...
          for (int jj = 0; jj < 3; jj++){
            ...
            mat[ii][jj] = FC.matMin2Plus2f[j*3*3 + ii*3 +jj];
          }
        }
...
void init_fault_coef(realptr_t M, Fault FC){
  ...
  init_fault_coef_cu <<<grid, block>>> (M, FC);
}
