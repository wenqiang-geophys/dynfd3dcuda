...
void trial_sw(Wave W, Fault F, real_t *M, int it, int irk, int FlagX, int FlagY, int FlagZ)
{
  dim3 block(16, 8, 1);
  dim3 grid(
      (hostParams.nj+block.x-1)/block.x,
      (hostParams.nk+block.y-1)/block.y, 1);
  trial_sw_cu <<<grid, block>>> (W, F, M, it, irk, FlagX, FlagY, FlagZ);
}
