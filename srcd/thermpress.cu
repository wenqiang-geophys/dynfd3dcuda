    ...
    int it, int irk, int FlagX, int FlagY, int FlagZ)
{
  ...
  if ( j < nj && k < nk && F.united[j + k * nj] == 0){  // not united
    ...
    for (int it = 0; it < nt_sub; it++){
      real_t tau = -friction * (Tau_n + F.TP_P[j+k*nj+0*nj*nk]);
      ...
      for (int i = 0; i < TP_n; i++){
        ...
        long pos_m = j + k*nj + (i-1)*nj*nk;
        long pos   = j + k*nj + (i  )*nj*nk;
        long pos_p = j + k*nj + (i+1)*nj*nk;
        if (i == 0){
...
void thermpress(Wave W,Fault F,real_t *M,int it,int irk,int FlagX,int FlagY,int FlagZ)
{
  dim3 block(16, 8, 1);
  dim3 grid(
      (hostParams.nj+block.x-1)/block.x,
      (hostParams.nk+block.y-1)/block.y, 1);
  thermpress_cu <<<grid, block>>> (W, F, M, it, irk, FlagX, FlagY, FlagZ);
}
